...
/*
	public void numberClick(ButtonClick[][] buttons, int y, int x) {
		int flagCount = 0;
		
		System.out.println("clicked on revealed number");
		//flag counter
		if (y > 0) {
			if (buttons[y - 1][x].subState == 1)
				flagCount++;
			if (x > 0) {
				if (buttons[y - 1][x - 1].subState == 1)
					flagCount++;
			}
		...
		//revealer
		if (flagCount == buttons[y][x].state && buttons[y][x].state != 0) {
			if (y > 0) {
				if (buttons[y - 1][x].subState == 0)
					buttons[y - 1][x].numberClicked();
				if (x > 0) {
					if (buttons[y - 1][x - 1].subState == 0)
						buttons[y - 1][x - 1].numberClicked();
				}
			...
			if (x > 0) {
				if (buttons[y][x - 1].subState == 0)
					buttons[y][x - 1].numberClicked();
			}
